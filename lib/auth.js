...
async function requireAuth(req) {
  ...
  try {
    ...
    // Tokens live 180 days with no revocation — after account deletion the old token otherwise
    // still "authenticates" and every route then dies on an FK violation against the vanished
    // user id (raw 500s instead of a clean 401 that signs the client out).
    const { sql } = require('./db');
    const { rows } = await sql`SELECT 1 FROM users WHERE id = ${payload.sub}`;
    return rows.length > 0 ? payload.sub : null;
  } catch {
    return null;
  }
}
