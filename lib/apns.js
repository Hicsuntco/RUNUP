...
async function sendPush(deviceToken, { title, body, sound = 'default' } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (result) => {
      if (settled) return;
      settled = true;
      // Close on EVERY path — the error paths used to skip this, leaking HTTP/2 sessions.
      try { if (client) client.close(); } catch { /* already closed */ }
      resolve(result);
    };

    client = http2.connect(`https://${APNS_HOST}`);
    client.on('error', () => finish({ ok: false, shouldDelete: false }));
    // Pushes are awaited BEFORE the API response (Vercel freeze semantics) — a hung APNs
    // connection without a timeout stalls the whole create/kudos/comment call until the
    // function times out, and the client then retries a post that actually succeeded.
    client.setTimeout(3000, () => finish({ ok: false, shouldDelete: false }));
...
// Fan-out to many users with ONE device_tokens query instead of one per member — a 50-member
// club post used to open 50 sequential SELECTs (plus a connection per device) before the API
// call could respond. Dead tokens are cleaned up in one DELETE at the end.
async function sendPushToUsers(sql, userIds, notification) {
  if (!userIds.length) return;
  try {
    const { rows } = await sql`SELECT token FROM device_tokens WHERE user_id = ANY(${userIds})`;
    const dead = [];
    await Promise.all(
      rows.map(async (row) => {
        const result = await sendPush(row.token, notification);
        if (result.shouldDelete) dead.push(row.token);
      })
    );
